...
public class ServiceOrderPaymentApi {
    ...
    public ResponseEntity<?> ready(@CurrentAccount Member member, @PathVariable Long id, @RequestBody ServiceOrderPaymentReadyDto.Request dto, Model model) {
        ServiceOrderPaymentReadyDto.Response response = serviceOrderPaymentService.ready(id, dto, member);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/complete")
    public ResponseEntity<?> complete(@CurrentAccount Member member, @PathVariable Long id, @RequestBody ServiceOrderPaymentReadyDto.Request dto, Model model) {
        ServiceOrderPaymentReadyDto.Response response = serviceOrderPaymentService.ready(id, dto, member);
        return ResponseEntity.ok(response);
    }
